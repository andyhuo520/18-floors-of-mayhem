...
def collect(p):
 ...
 if not p.read_text().strip():
  r=subprocess.run(['combos','asset','generate','--type','static_image','--prompt',(root/(key+'.prompt.txt')).read_text(),'--async','--idempotency-key','man18-skills-v4-'+key,'--json','--quiet'],capture_output=True,text=True)
...
with concurrent.futures.ThreadPoolExecutor(max_workers=6) as pool:list(pool.map(collect,[root/(key+'.json') for key in ['heroes-a','heroes-b','boss-a','boss-b','boss-c','impacts']]))
